...
- **テスト名の明確化**
  ...
  - 良い例: `test('有効なメールアドレスでユーザーを作成すると、IDが生成されて保存される', ...)`
- **モックの適切な使用**
  - 必要最小限のモック化
  - 外部依存の適切な分離
  - 実装詳細ではなく振る舞いをテストする
...
- **1 テスト 1 観点**
  - 1 つのテストで 1 つの振る舞いのみを検証
  - 複数のアサーションは関連する検証に限定
- **テストの独立性**
  - テスト間でデータや状態を共有しない
  - setUp/tearDown で適切にテスト環境をリセット
  - 実行順序に依存しない設計
- **アサーションの品質**
  - 曖昧な検証を避ける（`toBeTruthy()` → `toBe(true)` や具体的な値）
  - 境界値テストを含める
  - 異常系テストを適切に含める
- **テストデータの管理**
  - マジックナンバーや意味不明な値を避ける
  - ファクトリ関数やビルダーパターンで意図が明確なテストデータを作成
  - 例: `createValidUser()`, `createMinorUser()` など
