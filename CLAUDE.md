...
│   ├── completed/          # 役目を終えた完了ドキュメント（歴史的記録のみ）
...
- **completed/**: 実装完了したドキュメント（今後も参照価値があるもの）
- **adr/**: アーキテクチャ上の重要な決定事項の記録（技術選定、設計方針など）
- **archive/**: 過去のドキュメント（役目を終えた完了記録、非推奨になった内容、却下された提案）
...
- **implementation/** 配下は機能やコンポーネントの種類に応じてフォルダを作成
  ...
  - 実装完了後は `completed/` へ移動
- **design/** は設計段階のドキュメントを管理
  - 実装開始時に `design/` から `implementation/` へ移動
  - 提案段階は `proposals/`、採用された設計は `current/` に配置
- **completed/** は実装完了したドキュメントを管理
  - 機能カテゴリごとにフォルダを作成（implementation/と同じ構造）
  - 今後も参照価値のあるドキュメントのみを配置
    - 参照価値の判断基準例：
      - 現在のコードベースで使用されている機能や仕様の説明
      - 将来の開発で参考になる設計パターンや技術的な解決策
      - 開発者が頻繁に参照する技術ノートや運用手順
  - 役目を終えたドキュメントは `archive/completed/` へ移動
- **adr/** はアーキテクチャの重要な決定事項を記録
  - 番号付きファイル名で管理（例：`0001-採用した技術.md`）
  - 決定の背景、理由、影響を明確に記載
- **archive/** は不要になったドキュメントを保管
  - `completed/`: 役目を終えた完了ドキュメント（歴史的記録として保持）
