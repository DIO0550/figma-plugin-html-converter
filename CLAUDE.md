...
```
docs/
├── implementation/          # 実装完了した機能のドキュメント（参照価値あり）
...
│   ├── completed/          # 実装完了して役目を終えたもの（歴史的記録）
...
- **implementation/**: 現在のコードベースに関連する実装ドキュメント（今後の開発や保守で活用される内容）
- **design/**: 設計段階のドキュメント（実装前の提案や現在採用している設計）
- **adr/**: アーキテクチャ上の重要な決定事項の記録（技術選定、設計方針など）
- **archive/**: 過去のドキュメント（実装完了後の記録、非推奨になった内容、却下された提案）
...
- **implementation/** 配下は機能やコンポーネントの種類に応じてフォルダを作成
  ...
  - 例：HTML要素別、レイアウト機能、スタイルマッピング、データモデルなど
  - 現在も参照価値のあるドキュメントのみを配置
    - 参照価値の判断基準例：
      - 現在のコードベースで使用されている機能や仕様の説明
      - 将来の開発で参考になる設計パターンや技術的な解決策
      - 開発者が頻繁に参照する技術ノートや運用手順
    - 上記に該当しない、過去の情報や不要になったものは `archive/` へ移動
- **design/** は設計段階のドキュメントを管理
  - 実装完了後は `design/` から `implementation/` または `archive/completed/` へ移動
    - 移動判断基準：
      - 参照価値が残る場合（今後の開発や保守で活用される内容）は `implementation/` へ移動
      - 役目を終え、今後参照される可能性が低い場合は `archive/completed/` へ移動（歴史的記録として保持）
      - 例：汎用的な設計ガイドや再利用可能な仕様は `implementation/`、一度限りの設計や完了した機能の詳細は `archive/completed/`
  - 提案段階は `proposals/`、採用された設計は `current/` に配置
- **adr/** はアーキテクチャの重要な決定事項を記録
  - 番号付きファイル名で管理（例：`0001-採用した技術.md`）
  - 決定の背景、理由、影響を明確に記載
- **archive/** は不要になったドキュメントを保管
  - `completed/`: 実装完了して役目を終えたドキュメント（歴史的記録として保持）
