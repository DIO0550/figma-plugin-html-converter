...
describe("SVG基本図形 統合テスト", () => {
  describe("複数図形の同時変換", () => {
    test("circle, rect, line, ellipseを同時に変換する", () => {
      // 各要素を作成
  ...
  describe("mapToFigma関数の互換性", () => {
    test("HTMLNodeライクなオブジェクトを各要素タイプで正しくマッピングする", () => {
      const circleNode = {
        type: "element",
        tagName: "circle",
  ...
  describe("座標変換の正確性", () => {
    test("中心座標からの変換が正しい（circle, ellipse）", () => {
      const circle = CircleElement.create({
        cx: 100,
        cy: 100,
        r: 50,
      ...
      // circleの境界ボックス: x=50, y=50, width=100, height=100
      ...
      // ellipseの境界ボックス: x=20, y=60, width=160, height=80
