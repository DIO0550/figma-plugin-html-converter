...
test("PAttributes - グローバル属性を含むことができる", () => {
...
test("PAttributes - 空の属性オブジェクトを作成できる", () => {
  const attributes: PAttributes = {};
  expect(attributes).toEqual({});
});

test("PAttributes - 部分的な属性セットを作成できる", () => {
  const attributes: PAttributes = {
    id: "para-1",
...
test("PAttributes - カスタムdata属性を含むことができる", () => {
  const attributes: PAttributes = {
...
test("PAttributes - アクセシビリティ属性を含むことができる", () => {
  const attributes: PAttributes = {
...
test("PAttributes - イベントハンドラ属性を含むことができる", () => {
  const attributes: PAttributes = {
    onclick: "handleClick()",
...
test("PAttributes - GlobalAttributesと互換性がある", () => {
  const globalAttrs = {
    id: "test",
    class: "test-class",
    style: "color: red;",
