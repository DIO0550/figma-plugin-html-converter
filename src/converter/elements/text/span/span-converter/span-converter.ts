...
export const SpanConverter = {
  ...
  toFigmaNode(element: SpanElement): TextNodeConfig {
    ...
    // ベースのテキストノード設定を作成
    const baseConfig = TextNodeConfig.create(
      HTMLNode.extractTextFromNodes(element.children || []),
    );
    const withName = { ...baseConfig, name: buildNodeName(element) };

    // Typographyを利用して統一的に適用（タグはspan）
    return Typography.applyToTextNode(withName, styles, "span");
  },
