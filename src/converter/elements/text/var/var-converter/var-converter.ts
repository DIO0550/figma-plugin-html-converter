...
import { VarElement as VarElementHelper } from "../var-element";
...
/**
 * VarConverter
 * var要素をFigmaのTEXTノードに変換します
 */
export const VarConverter = {
  /**
   * var要素をFigmaノードに変換
   */
  toFigmaNode(element: VarElement): TextNodeConfig {
  ...
  /**
   * 汎用的なノードをvar要素としてFigmaノードにマッピング
   */
  mapToFigma(node: unknown): FigmaNodeConfig | null {
    if (!VarElementHelper.isVarElement(node)) {
      return null;
    }
