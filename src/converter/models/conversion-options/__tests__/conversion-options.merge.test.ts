...
test("nullが渡された場合にベース値が保持される（外部入力のnull混入対策）", () => {
  const base = ConversionOptions.getDefault();
  // 外部入力経由でnullが混入するケースを想定（型を強制キャスト）
  const override = {
    spacing: null as unknown as undefined,
    containerWidth: null as unknown as undefined,
    containerHeight: null as unknown as undefined,
  };
