...
test("ベースオプションとオーバーライドをマージできる", () => {
  ...
  expect(merged.containerHeight).toBe(defaults.containerHeight);
  expect(merged.spacing).toBe(defaults.spacing);
});
...
test("明示的undefinedを渡した場合にベース値が保持される", () => {
  ...
  expect(merged.spacing).toBe(defaults.spacing);
  expect(merged.containerWidth).toBe(defaults.containerWidth);
  expect(merged.containerHeight).toBe(defaults.containerHeight);
});
