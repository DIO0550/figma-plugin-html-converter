...
test("startHttp: handleRequestが非Errorオブジェクトをthrowした時にelse分岐のエラーログが出力される", async () => {
  const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

  const thrownValue = "unexpected failure";
  mockHandleRequest.mockRejectedValueOnce(thrownValue);

  await startHttp(3000);

  const mockReq = { body: {} };
  const mockRes = { on: vi.fn() };
  await expect(mockPostHandler!(mockReq, mockRes)).rejects.toBe(thrownValue);
...
test("startHttp: server.close()がcleanup中に失敗した時にエラーログが出力される", async () => {
  const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
...
test("startHttp: transport.close()とserver.close()が同時に失敗した時にエラーログが2回出力される", async () => {
  const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

  const transportCloseError = new Error("transport close failed");
  const serverCloseError = new Error("server close failed");
  mockTransportClose.mockRejectedValueOnce(transportCloseError);
  mockClose.mockRejectedValueOnce(serverCloseError);
  ...
  await vi.waitFor(() => {
    const cleanupCalls = errorSpy.mock.calls.filter(
      (call) => call[0] === "MCPクリーンアップエラー:",
    );
