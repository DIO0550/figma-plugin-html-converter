...
describe("startStdio", () => {
  test("StdioServerTransportを作成してconnectを呼ぶ", async () => {
  ...
  test("stderrにstdio起動ログを出力する", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
...
describe("startHttp", () => {
  test("createMcpExpressAppとlistenを呼ぶ", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
  ...
  test("stderrにHTTP起動ログを出力する", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
  ...
  test("/mcp ハンドラがStreamableHTTPServerTransportを作成する", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
