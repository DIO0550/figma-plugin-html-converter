...
describe("handleConvertHtml", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("正常なHTML入力でFigmaNodeConfig JSONを返す", async () => {
    const mockResult = {
      type: "FRAME",
      name: "div",
      width: 800,
  ...
  it("containerWidth/Heightオプションが反映される", async () => {
    const mockResult = {
      type: "FRAME",
      name: "div",
      width: 1024,
  ...
  it("1MBを超えるHTMLでサイズ超過エラーを返す", async () => {
  ...
  it("ちょうど1MBのHTML（境界値）は正常処理される", async () => {
  ...
  it("converter例外時にisError: trueのエラー応答を返す", async () => {
  ...
  it("レスポンス形式が正しい（content[0].type === 'text'）", async () => {
  ...
  describe("サロゲートペア文字の境界値テスト", () => {
    ...
    it("524,289個の絵文字（= 1,048,578 code units）は超過エラーになる", async () => {
      const emoji = "\u{1F600}";
  ...
  it("Error以外の例外もstring化してエラー応答を返す", async () => {
